...
module.exports = function(grunt) {
	...
	grunt
			.initConfig({
				...
				nodemon : {
					dev : {
						...
						options : {
							...
							// omit this property if you aren't serving HTML
							// files and
				...
				/*
				 * wiredep: { options: { cwd: '<%= yeoman.app %>' }, app: {
				...
				// ngAnnotate tries to make the code safe for minification
				// automatically by
	...
	grunt.registerTask('serve', 'Compile then start a connect web server',
			function(target) {
				...
				grunt.task.run([ 'clean:server', 'concurrent:server',
						'autoprefixer', 'nodemon', 'watch' ]);
			});
	...
	grunt.registerTask('build', [ 'clean:dist', 'concurrent:dist', 'copy:dist',
			'cssmin', 'ngAnnotate', 'uglify', 'htmlmin' ]);
